...
function mutationToOverrides(m: Mutation, opts: OverrideBuildOptions | undefined): TargetedOverride[] {
  ...
  switch (m.kind) {
    ...
    case "expense_one_time": {
      const age = clampAge(m.age, opts);
      const monthlyDelta = Number.isFinite(m.amountAnnual) ? m.amountAnnual / 12 : 0;
      pushAdd(out, "spend.lifestyle", age, monthlyDelta, age);
      break;
    }

    case "expense_recurring": {
      const startAge = clampAge(m.startAge, opts);
      const endAge =
        m.endAge != null && Number.isFinite(m.endAge)
          ? clampAge(m.endAge, opts)
          : (opts?.maxAge ?? 120);
      const monthlyDelta = Number.isFinite(m.amountAnnual) ? m.amountAnnual / 12 : 0;
      const effectiveEnd = Math.max(startAge, endAge);
      for (let a = startAge; a <= effectiveEnd; a++) {
        pushAdd(out, "spend.lifestyle", a, monthlyDelta, a);
      }
      break;
    }
    ...
    case "home_purchase": {
      const fromAge = clampAge(m.age, opts);
      const ti = Number.isFinite(m.taxesInsuranceMonthly) ? Math.max(0, m.taxesInsuranceMonthly) : 0;
      if (m.replaceExistingHousing) {
        pushSet(out, "spend.housing", fromAge, ti, undefined);
      } else {
        pushAdd(out, "spend.housing", fromAge, ti, undefined);
      }
      break;
    }
