...
import type { ExpenseImpactKind } from "./ExpenseImpactPicker";
...
import { ExpenseImpactPicker } from "./ExpenseImpactPicker";
import { ExpenseOneTimeEditor } from "./ExpenseOneTimeEditor";
import { ExpenseRecurringEditor } from "./ExpenseRecurringEditor";
import { HomePurchaseEditor, makeDefaultHomePurchaseMutation } from "./HomePurchaseEditor";
...
type TabId = "income" | "expenses" | "debt" | "housing";
...
export function LifeEventModal({
  ...
  const [iconKey, setIconKey] = useState<LifeEventIconKey>("custom");
  ...
  const housingMenuRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    if (event !== null) {
      ...
      setIconKey(event.iconKey ?? "custom");
  ...
  const handlePickExpenseImpact = (kind: ExpenseImpactKind) => {
    if (kind === "expense_one_time") {
      const m: ExpenseOneTimeMutation = {
      ...
      const m: ExpenseRecurringMutation = {
  ...
  const isDuplicateTitle =
    ...
    existingEvents.some(
      (evt) =>
        evt.id !== event?.id && normalizeTitle(evt.title) === normalizeTitle(trimmedTitle),
    );

  const handleSave = () => {
    const trimmed = title.trim();
    if (!trimmed || isDuplicateTitle) return;
    if (isCreate) {
  ...
  const hasCaps = mutations.some((m) => m.kind === "income_cap_range");
  const hasGrowthSteps = mutations.some((m) => m.kind === "income_growth_step");
  const hasHomePurchase = mutations.some((m) => m.kind === "home_purchase");
  const showHomeSection = iconKey === "home" || hasHomePurchase;
  ...
  const hasExpenseMutations = mutations.some(
    (m) => m.kind === "expense_one_time" || m.kind === "expense_recurring",
  );
  ...
  const renderExpenseContent = () => (
    <>
      {!hasExpenseMutations && (
        <p className="text-sm text-muted-foreground">
          No expense impacts yet. Add an expense impact below.
        </p>
      )}
      <ExpenseOneTimeEditor mutations={mutations} onChange={setMutations} />
      <ExpenseRecurringEditor mutations={mutations} onChange={setMutations} />
      <div className="mt-2">
        <ExpenseImpactPicker
          onPick={handlePickExpenseImpact}
          buttonLabel="Add expense impact"
        />
      </div>
    </>
  );
  ...
  useEffect(() => {
    if (!open || !housingMenuOpen) return;
    const handleClickOutside = (event: MouseEvent) => {
      const target = event.target as Node;
      if (!housingMenuRef.current?.contains(target)) {
  ...
  const handleAddPrimaryResidence = () => {
    setIconKey("home");
    setMutations((prev) =>
      prev.some((m) => m.kind === "home_purchase")
        ? prev
        : [...prev, makeDefaultHomePurchaseMutation()],
    );
    setHousingMenuOpen(false);
  };
  ...
  const renderHousingContent = () => (
    <div className="space-y-3">
      <div className="relative inline-flex" ref={housingMenuRef}>
        <Button
          variant="outline"
          size="sm"
        ...
        {housingMenuOpen && (
          <div className="absolute left-0 top-full z-20 mt-2 w-72 rounded-md border border-border bg-background shadow-md">
            <button
              type="button"
              className="w-full px-3 py-2 text-left text-sm hover:bg-muted/40"
              onClick={handleAddPrimaryResidence}
            >
              Purchase a Primary Residence
            </button>
            <div className="w-full px-3 py-2 text-left text-sm text-muted-foreground cursor-not-allowed opacity-60">
              Purchase a Secondary Residence / Investment Property
            </div>
          </div>
        )}
      </div>
      {hasHomePurchase && (
        <HomePurchaseEditor mutations={mutations} onChange={setMutations} />
      )}
    </div>
  );
  ...
  const modalContent = (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      <div
        ...
        aria-hidden
      />
      <Card className="relative z-10 w-full max-w-3xl mx-4 max-h-[90vh] flex flex-col bg-background shadow-lg">
        ...
        <CardContent className="space-y-4 overflow-y-auto">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              className={cn(
                isDuplicateTitle && "animate-shake border-destructive focus-visible:ring-destructive",
              )}
            />
            {isDuplicateTitle && (
              <p className="text-xs text-muted-foreground">name already in use</p>
            )}
          </div>
          <div className="space-y-2">
            <Label>Icon</Label>
            <div className="grid grid-cols-4 gap-2">
              {LIFE_EVENT_ICON_KEYS.map((key) => {
                ...
                return (
                  <button
                    ...
                    title={config.label}
                  >
                    <LifeEventIcon iconKey={key} size="md" />
          ...
          <div className="flex flex-wrap items-center gap-2 pt-2">
            <Button onClick={handleSave} disabled={!title.trim() || isDuplicateTitle}>
              Save
            </Button>
